...
describe("useView integration", () => {
  ...
  it("prop sync updates reactively", () => {
    const vmContract = createContract()
      .store("n", (s) => s<number>().default(0));
    const vm = createViewModel({
      contract: vmContract,
    ...
    function Display() {
      ...
      const view = useUnit(useView(vm, () => ({ input: val() }))) as any;
...
describe("useView props", () => {
  ...
  it("multiple store props change independently", () => {
    ...
    const setFirst = createEvent<string>();
    const setSecond = createEvent<number>();
    const $first = createStore("a").on(setFirst, (_, v) => v);
    const $second = createStore(1).on(setSecond, (_, v) => v);

    function Comp() {
      const first = useUnit($first);
      const second = useUnit($second);
      const view = useUnit(useView(vm, () => ({ first: first(), second: second() }))) as any;
      return (
        <div>
  ...
  it("store prop survives rapid changes", () => {
    ...
    const setVal = createEvent<number>();
    const $val = createStore(0).on(setVal, (_, v) => v);

    function Comp() {
      const val = useUnit($val);
      const view = useUnit(useView(vm, () => ({ val: val() }))) as any;
  ...
  it("store prop feeds internal derived state", () => {
    ...
    const setMult = createEvent<number>();
    const $mult = createStore(3).on(setMult, (_, v) => v);

    function Comp() {
      const mult = useUnit($mult);
  ...
  it("mixed store and event props work together", () => {
    ...
    const $title = createStore("Draft").on(setTitle, (_, v) => v);
